...
cnpy::npy_array load_the_npz_array(FILE* fp, uint32_t compr_bytes, uint32_t uncompr_bytes)
{
...
cnpy::npy_array cnpy::npz_load(std::string fname, std::string varname)
{
        ...
        while (1)
        {
                ...
                if (vname == varname)
                {
                        npy_array array = (compr_method == 0) ? load_the_npy_file(fp) : load_the_npz_array(fp, compr_bytes, uncompr_bytes);
                        fclose(fp);
                        return array;
                }
                else
